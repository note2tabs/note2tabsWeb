...
export function buildPaymentFailedEmail(input: { name?: string | null }) {
  ...
  const text = `${greeting}\n\nWe couldn't process your latest Note2Tabs Premium payment. Your Premium access remains available during a ${PREMIUM_PAYMENT_GRACE_DAYS}-day recovery period.\n\nUpdate your payment method: ${settingsUrl}\n\nIf the payment is not resolved during that period, Premium will end automatically. You can still use Note2Tabs on the Free plan.`;
  const html = `
    <div style="background:#f7f4ec;padding:32px 16px;font-family:Arial,sans-serif;color:#07110e">
      <div style="max-width:600px;margin:0 auto;background:#fff;border:1px solid #dedbd2;border-radius:20px;padding:32px">
        ...
        <h1 style="font-size:28px;line-height:1.15;margin:0 0 16px">Your Premium payment needs attention</h1>
        <p style="font-size:16px;line-height:1.6;color:#59615d;margin:0 0 24px">We couldn't process your latest payment. Your Premium access remains available during a ${PREMIUM_PAYMENT_GRACE_DAYS}-day recovery period.</p>
        ...
        <p style="font-size:14px;line-height:1.5;color:#747b77;margin:24px 0 0">If it isn't resolved during that period, Premium will end automatically. Your account and tabs will remain available on the Free plan.</p>
      </div>
    </div>`;
