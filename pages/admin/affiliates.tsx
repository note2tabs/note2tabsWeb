...
export default function AdminAffiliatesPage({ initialAffiliates, initialPromotions }: { initialAffiliates: AffiliateRow[]; initialPromotions: PromotionRow[] }) {
  ...
  const copyLink = async (affiliate: AffiliateRow) => { await navigator.clipboard.writeText(`${window.location.origin}/?ref=${affiliate.code}`); setCopiedId(affiliate.id); window.setTimeout(() => setCopiedId(null), 1800); };
