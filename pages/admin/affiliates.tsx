...
type PromotionRow = { id: string; code: string; active: boolean; percentOff: number; durationMonths: number; expiresAt: number | null; timesRedeemed: number; maxRedemptions: number | null; cardFreeSchoolAccess: boolean };
...
export default function AdminAffiliatesPage({ initialAffiliates, initialPromotions }: { initialAffiliates: AffiliateRow[]; initialPromotions: PromotionRow[] }) {
  ...
  const [commissionMonths, setCommissionMonths] = useState("6");
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      const terms = { commissionPercent: Number(commissionPercent), commissionMonths: Number(commissionMonths), discountPercent: Number(discountPercent), discountMonths: Number(discountMonths) };
      const response = await fetch("/api/admin/affiliates/invite", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ email, code, ...terms }) });
      const body = await response.json();
      if (response.ok) {
        setMessage({ text: `Created affiliate ${body.affiliate.code}.`, tone: "success" });
        setAffiliates((rows) => [{ id: body.affiliate.id, code: body.affiliate.code, status: body.affiliate.status, email, referrals: 0, pendingCommissions: 0, ...terms }, ...rows]);
        setEmail(""); setCode("");
  ...
  const createPromotion = async (event: FormEvent) => {
    ...
    try {
      const response = await fetch("/api/admin/affiliates/promotions/create", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ code: promotionCode, percentOff: Number(percentOff), durationMonths: Number(durationMonths), expiresOn, maxRedemptions: maxRedemptions ? Number(maxRedemptions) : null, cardFreeSchoolAccess }) });
      const body = await response.json();
      if (response.ok) {
        setPromotions((rows) => [body.promotion, ...rows]); setPromotionCode(""); setExpiresOn(""); setMaxRedemptions("");
  ...
  return <><Head><title>Affiliates | Note2Tabs Admin</title><meta name="robots" content="noindex,nofollow" /></Head><main className="adminAffiliatePage"><div className="adminAffiliateShell">
    ...
    <div className="adminAffiliateLayout"><section className="adminAffiliateCard adminAffiliateInvite"><div className="adminAffiliateCardHeading"><div><span className="affiliateSectionLabel">New partnership</span><h2>Invite an affiliate</h2></div><span className="adminAffiliateStep">Stripe managed</span></div><p>The person needs an existing Note2Tabs account. Stripe will securely handle their identity and payout details.</p><form onSubmit={submit}><div className="adminAffiliateField"><label htmlFor="affiliate-email">Account email</label><input id="affiliate-email" type="email" required autoComplete="email" placeholder="artist@example.com" value={email} onChange={(event) => setEmail(event.target.value)} /></div><div className="adminAffiliateField"><label htmlFor="affiliate-code">Referral and coupon code</label><input id="affiliate-code" required minLength={3} maxLength={32} placeholder="ARTIST20" value={code} onChange={(event) => setCode(event.target.value.toUpperCase().replace(/[^A-Z0-9_-]/g, ""))} /><small>3–32 characters. Letters, numbers, dashes, and underscores.</small></div><fieldset className="adminAffiliateTermFields"><legend>Custom terms</legend><div className="adminAffiliateField"><label htmlFor="affiliate-commission">Affiliate earns</label><div className="adminAffiliateInputSuffix"><input id="affiliate-commission" type="number" min="1" max="100" step="1" required value={commissionPercent} onChange={(event) => setCommissionPercent(event.target.value)} /><span>%</span></div></div><div className="adminAffiliateField"><label htmlFor="affiliate-commission-months">Earning period</label><div className="adminAffiliateInputSuffix"><input id="affiliate-commission-months" type="number" min="1" max="24" step="1" required value={commissionMonths} onChange={(event) => setCommissionMonths(event.target.value)} /><span>months</span></div></div><div className="adminAffiliateField"><label htmlFor="affiliate-discount">Customer saves</label><div className="adminAffiliateInputSuffix"><input id="affiliate-discount" type="number" min="1" max="100" step="1" required value={discountPercent} onChange={(event) => setDiscountPercent(event.target.value)} /><span>%</span></div></div><div className="adminAffiliateField"><label htmlFor="affiliate-discount-months">Discount period</label><div className="adminAffiliateInputSuffix"><input id="affiliate-discount-months" type="number" min="1" max="24" step="1" required value={discountMonths} onChange={(event) => setDiscountMonths(event.target.value)} /><span>months</span></div></div></fieldset><button className="affiliatePrimaryButton adminAffiliateSubmit" disabled={busy}>{busy ? "Creating affiliate…" : "Create affiliate"}<span aria-hidden="true">→</span></button></form></section>
    ...
    <section className="adminAffiliateCard adminAffiliatePromotion"><div className="adminAffiliateCardHeading"><div><span className="affiliateSectionLabel">General promotions</span><h2>Create a discount code</h2></div><span className="adminAffiliateStep">No commission</span></div><p>This code works at checkout but is not connected to an affiliate, referral, or commission. The end date and usage limit are optional.</p><form onSubmit={createPromotion}>
      ...
      <div className="adminAffiliateField"><label htmlFor="promotion-limit">Usage limit <em>Optional</em></label><div className="adminAffiliateInputSuffix"><input id="promotion-limit" type="number" min="1" max="1000000" step="1" placeholder="Unlimited" value={maxRedemptions} onChange={(event) => setMaxRedemptions(event.target.value)} /><span>uses</span></div></div>
...
export const getServerSideProps: GetServerSideProps = async (context) => {
  ...
  const affiliates = await prisma.affiliate.findMany({ orderBy: { createdAt: "desc" }, select: { id: true, code: true, status: true, commissionPercent: true, commissionMonths: true, discountPercent: true, discountMonths: true, owner: { select: { email: true } }, _count: { select: { attributions: true } }, commissions: { where: { status: "PENDING" }, select: { id: true } } } });
  ...
  if (stripeClient) {
    try {
      ...
      promotions = result.data.filter((promotion) => promotion.metadata?.[STANDALONE_PROMOTION_METADATA_KEY] === "true").map((promotion) => {
        ...
        return { id: promotion.id, code: promotion.code, active: promotion.active, percentOff: coupon?.percent_off || 0, durationMonths: coupon?.duration_in_months || 0, expiresAt: promotion.expires_at || null, timesRedeemed: promotion.times_redeemed || 0, maxRedemptions: promotion.max_redemptions || null, cardFreeSchoolAccess: promotion.metadata?.[SCHOOL_ACCESS_METADATA_KEY] === "true" || coupon?.metadata?.[SCHOOL_ACCESS_METADATA_KEY] === "true" };
      });
    } catch (error) { console.error("standalone promotions could not be loaded", error); }
  }
  return { props: { initialAffiliates: affiliates.map((affiliate) => ({ id: affiliate.id, code: affiliate.code, status: affiliate.status, email: affiliate.owner.email, referrals: affiliate._count.attributions, pendingCommissions: affiliate.commissions.length, commissionPercent: affiliate.commissionPercent, commissionMonths: affiliate.commissionMonths, discountPercent: affiliate.discountPercent, discountMonths: affiliate.discountMonths })), initialPromotions: promotions } };
};
