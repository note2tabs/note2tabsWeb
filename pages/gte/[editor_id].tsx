...
const normalizeCanvas = (raw: unknown, fallbackCanvasId: string): CanvasSnapshot => {
  if (isCanvasSnapshot(raw)) {
    ...
    return {
      ...
      activeLaneId: typeof raw.activeLaneId === "string" ? raw.activeLaneId : undefined,
...
export default function GteEditorPage({ editorId, isGuestMode }: Props) {
  ...
  const activeLaneHydratedRef = useRef(false);
  ...
  useEffect(() => {
    if (isGuestMode || !activeLaneId || !activeLaneHydratedRef.current) return;
    if (activeLaneSaveTimerRef.current !== null) {
      window.clearTimeout(activeLaneSaveTimerRef.current);
    }
    activeLaneSaveTimerRef.current = window.setTimeout(() => {
      activeLaneSaveTimerRef.current = null;
      void gteApi.setActiveLane(editorId, activeLaneId).catch(() => {});
    }, CONTROL_COMMIT_DEBOUNCE_MS);
    return () => {
      if (activeLaneSaveTimerRef.current !== null) {
        window.clearTimeout(activeLaneSaveTimerRef.current);
  ...
  }, [activeLaneId, editorId, isGuestMode]);
  ...
  const resolveActiveLaneId = useCallback(
    (prev: string | null, loadedCanvas: CanvasSnapshot) => {
      if (prev && loadedCanvas.editors.some((lane) => lane.id === prev)) return prev;
      if (
        loadedCanvas.activeLaneId &&
        loadedCanvas.editors.some((lane) => lane.id === loadedCanvas.activeLaneId)
      ) {
        return loadedCanvas.activeLaneId;
      }
      return loadedCanvas.editors[0]?.id || null;
    },
    []
  );
  ...
  const loadEditor = async () => {
    ...
    try {
      ...
      resetCanvasHistory();
      setActiveLaneId((prev) => resolveActiveLaneId(prev, normalized));
      activeLaneHydratedRef.current = true;
        ...
        >
          ...
          {!isGuestMode && (
            <button
              ...
              className="inline-flex h-9 items-center justify-center gap-1.5 rounded-lg border border-slate-300 bg-white px-3 text-xs font-semibold text-slate-800 shadow-sm transition hover:border-slate-400 hover:bg-slate-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-500 focus-visible:ring-offset-2 lg:w-full"
                  ...
                  <div className={`order-7 shrink-0 items-center gap-2 xl:ml-auto ${
                    ...
                    {!isGuestMode && (
                      <button
                        ...
                        className="inline-flex h-8 items-center gap-1.5 rounded-lg border border-slate-300 bg-white px-3 text-sm font-semibold text-slate-800 shadow-sm transition hover:border-slate-400 hover:bg-slate-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-500 focus-visible:ring-offset-2"
                        title="Share this tab"
                      >
                        <svg viewBox="0 0 20 20" className="h-4 w-4 fill-none stroke-current" aria-hidden="true">
                          <circle cx="5" cy="10" r="2" />
                    ...
                    {isGuestMode ? (
                      ...
                      <button
                        ...
                        className="inline-flex h-8 items-center rounded-lg border border-slate-200 bg-white px-3 text-sm font-medium text-slate-600 transition hover:border-slate-300 hover:bg-slate-50 hover:text-slate-800 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-400 focus-visible:ring-offset-2"
                      >
                        Back to editors
                      </button>
                    )}
